...
%>
	original movie_theater: <%=movie_theater_name %> / new movie_theater: <%=new_movie_theater_name %>
	<br/>
	address: <%=address %>
	<br/>
...
<%
	...
	if(modifiedSeat != null) {
		for(int i=0; i<modifiedSeat.length; i++) {
			String seat_id = modifiedSeat[i].split("==>>")[1];
			String seat_name = request.getParameter(modifiedSeat[i]);
%>
			수정: Seat: <%=seat_id %> / <%=seat_name %>
<%
		}
	}
...
%>
			수정: Theater: <%=theater_id %> / <%=theater_name %>
<%
		}
	}
